...
#include "main.h"
#include "pros/apix.h"
#include "vdml/registry.h"	//hidden header which determines what device is plugged in
...
public:
	...
	void setStyle(lv_color_t colorRel, lv_color_t colorPr, lv_color_t textColor)
	{
		...
		buttonStyleRel->body.main_color = buttonStyleRel->body.grad_color = colorRel;	//assign values to pointers
...
private:
	...
	std::vector<Line> yGuides, xGuides, line; //these are vectors of type Line
...
Button * box[24];	//Button class? points to an array
...
struct PortData
{
	...
	std::vector<TestPointResult> results;	//vector of structs (https://stackoverflow.com/questions/8067338/vector-of-structs-initialization
	long start = 0;
	int coastTime;
	int breakTime;
...
//creating the individual buttons
	for(int i = 0; i < 24; i++)
	{
